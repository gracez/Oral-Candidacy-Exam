...
Throughout this subsection, we will assume that $x_\ast$ is a stable rest point of an ODE. Probably the most classical mathematical definition of resilience, originally developed by theoretical ecologists\todo{citation}, represents long-term return rates to $x_{\ast}$, and is measured by the dominant eigenvalue at linearization. 
...
Note the expression $e^{tA}x$ in left hand side is exactly the flow for $x' = \mathbf{A}x$. So the theorem says that trajectories must decay to the origin at an exponential rate with a bound on that rate governed by $\lambda_1$. For nonlinear systems, a similar bound on decay rate is justified by the Stable Manifold Theorem. %, also a standard theoretical result. 
...
%A version of the theorem is built up below. 
