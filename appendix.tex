...
\label{stable_manifold_theorem}
 ...
 \begin{theorem}(Stable Manifold Theorem)
 	...
 	%
 	Also suppose $\mathbf{A}$ is infinitesimally hyperbolic. Let $E_s$ and $E_u$ be the stable and unstable eigenspaces of $\mathbf{A}$, respectively, so that $\textbf{A} = E_s \oplus E_u$. Let $P_s: \mathbb{R}^n \to E_s$ %and $P_u: \mathbb{R}^n \to E_u$ 
